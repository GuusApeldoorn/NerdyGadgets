...
public class Algorithm {
    ...
    String path = "C:\\Users\\Gerrit\\Desktop\\opslaan.txt";
    ...
    public void calculateAvailabilityPercentage(double AvailabilityPercentage) {
        ...
        for (int k = 1; k < nFactor; k++) {
            ...
            for (int i = 0; i < test.length; i++) {
                ...
                if ((mogelijkheden[0] == 0 && mogelijkheden[1] == 0 && mogelijkheden[2] == 0) || (mogelijkheden[3] == 0 && mogelijkheden[4] == 0 && mogelijkheden[5] == 0)) {
                    System.out.println();
                } else {

                    antwoord = ((firewall * dbloadbalancer) * (1 - ((Math.pow(1 - dbserver1, mogelijkheden[0])) * (Math.pow(1 - dbserver2, mogelijkheden[1])) * (Math.pow(1 - dbserver3, mogelijkheden[2]))))
                            * ((1 - ((Math.pow(1 - webserver1, mogelijkheden[3])) * (Math.pow(1 - webserver2, mogelijkheden[4])) * (Math.pow(1 - webserver3, mogelijkheden[5])))) * 100));

                    prijs = (int) ((firewall_prijs + dbloadbalancer_prijs) + (dbserver1_prijs * mogelijkheden[0]) + (dbserver2_prijs * mogelijkheden[1]) + (dbserver3_prijs * mogelijkheden[2])
                            + (webserver1_prijs * mogelijkheden[3]) + (webserver2_prijs * mogelijkheden[4]) + (webserver3_prijs * mogelijkheden[5]));

                    goedkoopstePrijs[1] = prijs; //prijs wordt telkens op plek 2 geplaats om vervolgens te vergelijken met plek 1.

                    if (antwoord >= AvailabilityPercentage && goedkoopstePrijs[0] == 0) { //initialiseren van plek 0 in array voor vergelijken
                    ...
                    if (goedkoopstePrijs[1] < goedkoopstePrijs[0] && antwoord >= AvailabilityPercentage) { //kijken of een gevonden mogelijkheid goedkoper is als plek 0 in de array qua prijs
            ...
            System.out.println(goedkoopstePrijs[0] + " " + mogelijkhedenGoedOntwerp[0] + "" + mogelijkhedenGoedOntwerp[1] + "" + mogelijkhedenGoedOntwerp[2] + "" + mogelijkhedenGoedOntwerp[3] + "" + mogelijkhedenGoedOntwerp[4] + "" + mogelijkhedenGoedOntwerp[5]);
            ...
            if (nFactor == 2) {
                nFactor++;
            } else if (goedkoopstePrijsVergelijken[k - 2] == goedkoopstePrijsVergelijken[k - 1] && (goedkoopstePrijsVergelijken[k - 2] != 0 || goedkoopstePrijsVergelijken[k - 1] != 0)) {
                break;
            } else {
                nFactor++;
            }
    ...
    public static String[][] calculateCombinations(int R, int N) {
        ...
        for (int i = 0; i < limit; i++) {                                       // over alle mogelijkheden iterateren
            ...
            for (int sub = R - base.length(); sub > 0; sub--) {                 // vul de waarde vanaf het begin in met nullen indien nodig
